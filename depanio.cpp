...
// write coded motion data to start of special clip frame buffer
//
void write_depan_data_one(uint8_t* dstp, int frame, float motionx, float motiony, float motionzoom)
{
    char signaturegood[8] = DEPANSIGNATURE;
    depanheader header;
    depandata framedata;
    ...
    header.nframes = 1; // number of frames to write motion info about
    ...
    framedata.frame = frame; // some frame number
    framedata.dx = motionx; // motion x for frame
    framedata.dy = motiony; // motion y for frame
    framedata.zoom = motionzoom; // zoom for frame
    framedata.rot = 0; // rotation for frame - not estimated in current version
    memcpy(dstp, &framedata, sizedata);
    dstp += sizedata;
}
